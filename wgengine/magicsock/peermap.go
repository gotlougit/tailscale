...
type peerMap struct {
	...
	// It is protected by nodMu to allow for modifications and cleanup of the
	// map and its sub Set's.
...
func (m *peerMap) upsertEndpoint(ep *endpoint, oldDiscoKey key.DiscoPublic,
	...
	epKey := epDisco.controlKey
	if fromTSMP {
		epKey = epDisco.tsmpKey
	}
...
func (m *peerMap) deleteEndpoint(ep *endpoint) {
	...
	if epDisco != nil {
		for _, discoKey := range []key.DiscoPublic{epDisco.controlKey, epDisco.tsmpKey} {
			if discoKey.IsZero() {
				continue
			}
			m.nodMu.Lock()
			s := m.nodesOfDisco[discoKey]
			delete(s, ep.publicKey)
			if len(s) == 0 {
				delete(m.nodesOfDisco, discoKey)
			}
			m.nodMu.Unlock()
		}
	}
